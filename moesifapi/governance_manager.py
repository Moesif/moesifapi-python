...
class GovernanceRulesManager:
  ...
  @classmethod
  def get_field_value_for_path(cls, path, request_fields={}, request_body={}, request_headers={}):
  ...
  def does_regex_config_match(self, regex_config, request_fields, request_body, request_headers):
    if not regex_config:
      return True

    def does_one_condition_match(condition):
      path = condition['path']
      field_value = self.get_field_value_for_path(path, request_fields, request_body, request_headers)
  ...
  def recursively_replace_values(self, temp_val, merge_tag_values={}, rule_variables=None):
    ...
    if type(temp_val) is dict:
      result = {}
      for key in temp_val:
        result[key] = self.recursively_replace_values(temp_val[key], merge_tag_values, rule_variables)
      return result

    if type(temp_val) is list:
      return map(lambda x: self.recursively_replace_values(x, merge_tag_values, rule_variables), temp_val)

    # for all other types just return value
    return temp_val

  def modify_response_for_one_rule(self, response_holder, rule, merge_tag_values):
    rule_variables = {}
    ...
    if 'response' in rule and 'headers' in rule['response']:
      ...
      if rule_headers:
        value_replaced_headers = self.recursively_replace_values(rule_headers, merge_tag_values, rule_variables)
    ...
    if 'block' in rule and rule['block']:
      ...
      response_holder['body'] = self.recursively_replace_values(rule_res_body, merge_tag_values, rule_variables)
      response_holder['status'] = rule['response']['status']

    return response_holder

  def apply_one_rule(self, response_holder, rule, config_rule_values):
    merge_tag_values = {}
    ...
    return self.modify_response_for_one_rule(response_holder, rule, merge_tag_values)

  def apply_rules(self, applicable_rules, response_holder, config_rules_values):
    try:
      if not applicable_rules:
        return response_holder

      for rule in applicable_rules:
        response_holder = self.apply_one_rule(response_holder, rule, config_rules_values)
  ...
  def get_applicable_regex_rules(self, request_fields, request_body, request_headers):
    if self.regex_rules:
      return filter(lambda rule: self.does_regex_config_match(rule['regex_config'], request_fields, request_body, request_headers), self.regex_rules)
    else:
      return []
  ...
  def get_applicable_unidentified_user_rules(self, request_fields, request_body, request_headers):
    if self.unidentified_user_rules:
      return filter(lambda rule: self.does_regex_config_match(rule['regex_config'], request_fields, request_body, request_headers), self.unidentified_user_rules)
    else:
      return []
  ...
  def get_applicable_unidentified_company_rules(self, request_fields, request_body, request_headers):
    if self.unidentified_company_rules:
      return filter(lambda rule: self.does_regex_config_match(rule['regex_config'], request_fields, request_body, request_headers), self.unidentified_company_rules)
    else:
      return []
  ...
  def get_user_rules(self, config_rules_values, request_fields, request_body, request_headers):
    ...
    if config_rules_values:
      for rules_values_entry in config_rules_values:
        ...
        regex_matched = self.does_regex_config_match(found_rule['regex_config'], request_fields, request_body, request_headers)
    ...
    for rule in self.user_rules.items():
      ...
      if rule_info['applied_to'] == 'not_matching' and not in_cohort_of_rule_hash.get(rule_info['_id'], None):
        regex_matched = self.does_regex_config_match(rule_info['regex_config'], request_fields, request_body, request_headers)
        if regex_matched:
  ...
  def prepare_request_fields(self, event_info, request_body):
    fields = {
  ...
  def govern_request(self, config, event_info, user_id, company_id, request_body, request_headers):
    ...
    response_holder = self.apply_rules(applicable_regex_rules, response_holder, None)
    ...
    if company_id is None:
      ...
      response_holder = self.apply_rules(unidentified_company_rules, response_holder, None)
    else:
      ...
      response_holder = self.apply_rules(company_rules, response_holder, config_rules_values)

    if user_id is None:
      unidentified_user_rules = self.get_applicable_unidentified_user_rules(request_fields, request_body, request_headers)
      response_holder = self.apply_rules(unidentified_user_rules, response_holder, None)
    else:
      ...
      response_holder = self.apply_rules(user_rules, response_holder, config_rules_values)

    if 'blocked_by' in response_holder:
      response_holder['body'] = self.format_body_for_middleware(response_holder['body'])
